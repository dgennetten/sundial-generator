/**
 * One-shot: upload sundial-settings-api.php to the production server.
 * Usage:  node scripts/upload-sundial-settings-api.js
 */
...
const LOCAL_FILE = path.join(__dirname, '..', 'sundial-settings-api.php');
const REMOTE_PATH = path.posix.join(DEPLOY_REMOTE_ROOT, 'sundial-settings-api.php');
...
try {
  ...
  console.log(`✅ Uploaded sundial-settings-api.php → ${REMOTE_PATH}`);
} finally {
  await sftp.end();
}
