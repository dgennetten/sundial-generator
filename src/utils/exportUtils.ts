...
const EXPORT_STALE_CHUNK_MESSAGE =
  'A new version of the sundial designer is available. The page will reload — please click Export again.';
...
export async function exportSundial(options: ExportOptions, onLogged?: () => void): Promise<void> {
  ...
  try {
    ...
    if (isStaleChunkError(error) && recoverFromStaleChunk(() => alert(EXPORT_STALE_CHUNK_MESSAGE))) {
      return;
    }
    throw error;
  }
