...
export function buildGnomonNetSVGString(
  ...
  const labelSize = Math.max(4, Math.min(gH * 0.22, 14));
  const p = {
    apex:  [gH,       0        ],
    left:  [0,        gH       ],
    right: [2 * gH,  gH       ],
  ...
  const netInner = [
    ...
    `      stroke="#333" stroke-width="0.35" stroke-dasharray="2,2"/>`,
    `<text x="${aCx}" y="${aCy}" font-size="${labelSize}" text-anchor="middle" dominant-baseline="middle"`,
    `      transform="rotate(45 ${aCx} ${aCy})" font-family="sans-serif" font-weight="bold">A</text>`,
    `<text x="${bCx}" y="${bCy}" font-size="${labelSize}" text-anchor="middle" dominant-baseline="middle"`,
    `      transform="rotate(-45 ${bCx} ${bCy})" font-family="sans-serif" font-weight="bold">B</text>`,
  ].join('\n      ');
